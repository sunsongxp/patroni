...
ETCD_ARGS="--data-dir /tmp/etcd.data --name={ETCD_HOSTNAME} -advertise-client-urls=${ADVERTISE_CLIENT_URLS} -listen-client-urls=${LISTEN_CLIENT_URLS} --initial-cluster-token=${INITIAL_CLUSTER_TOKEN} --initial-advertise-peer-urls=${INITIAL_ADVERTISE_PEER_URLS}"
